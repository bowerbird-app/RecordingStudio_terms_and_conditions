...
- **Admin** create/edit Terms, Publishable publish, and a Users page of receipts for each term. Engine tables use `TablePage` (Pagy, 25 rows) and Flatpack infinite pagination. Admin hub Terms and Who agreed tables set `paginate per_page: 25` the same way Recording Studio Admin dummy tables do. Dummy `/docs/gem_views` is the other table page and uses the same page size.
