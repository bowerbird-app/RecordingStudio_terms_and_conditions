...
6. The gem includes `ForcesAcceptance` on the host `ApplicationController` and prepends Users Auth `after_sign_in` / `after_sign_up` to the same Agree screen until live Terms are accepted. Do not add a second clickwrap. Drop `recording_studio_terms_agree` onto a host form (checkbox only); on submit call `accept!` for the pending live version. Or drop `recording_studio_terms_continue_notice` and call `accept!` with `{ "source" => "continue_notice" }`. If you override default layout, skip PageNav when `content_for?(:skip_page_nav)` is set.
