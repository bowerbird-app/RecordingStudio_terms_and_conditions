...
- Write Terms parks under Recording Studio Admin (`/admin`). Show pages put copy in the Flatpack content surface, print the published date under the title, and give the Agree checkbox more vertical space. `recording_studio_terms_agree(link_terms: true)` turns the word terms into a link to the live public page. Admin Terms tables include a Published column. Write/edit field spacing and the rich-text field border stay; only the old Card wrapper is gone.
