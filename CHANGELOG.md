...
- Write Terms parks under Recording Studio Admin (`/admin`). Show pages put copy in the Flatpack content surface, print the published date tight under the subtitle, and give the Agree checkbox more vertical space. `recording_studio_terms_agree(link_terms: true)` turns the word terms into a link to the live public page. Admin Terms tables include a Published column. Write/edit keep Title/Body spacing. The Body wrapper has no extra border; the rich-text field keeps its own chrome.
