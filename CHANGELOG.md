...
- Agree no longer shows the on-page “Terms updated” Flatpack Alert or the “You already agreed…” paragraph. Re-accept keeps the flash “We've updated our Terms and Conditions” and the calendar date subtitle. The mounted Accept screen uses `recording_studio_terms_continue_notice` (no checkbox) and a **Continue** button; POST writes `{ "source" => "continue_notice" }`.
